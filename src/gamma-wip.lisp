...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defun gamma-cdf (x a b)
  "Return the cumulative gamma distribution function, shape a>0, scale b>0"
  (cond ((or (< a 0)
	     (< b 0))
	     (bits-double-float #b0111111111111000000000000000000000000000000000000000000000000000)) ;double-float-nan
	 ((< x 0) 0)
	 (t (incomplete-gamma a (/ x b)))))


;; Convenience functions for p and q
(defun p-incomplete-gamma (x a)
...
P is the first value, Q the second, DAX the third, e.g. (values p q dax)"
  ...
  (let (p q
	...
	(when (not p)			;or q?
	  ;; (format t "dax: ~F p: ~F, q: ~F~%" dax p q)
	  (let (alfa)
	    (if (> x 0.25)
		(setf alfa (+ x 0.25d0))
		(setf alfa (- (/ (log 2d0)
				 (log x)))))
	    (cond ((> a alfa)
		    ;; (format t "using p-taylor~%")
		   (multiple-value-setq (p q)
		     (p-taylor a x dax)))
		  ((< x 1)
		    ;; (format t "using q-taylor~%")
		   (multiple-value-setq (p q)
		     (q-taylor a x)))
		  (t ;;(format t "using q-fraction~%")
		     (multiple-value-setq (p q)
		       (q-fraction a x dax)))))))

    ;; (format t "a: ~A, x: ~A, p: ~A, q: ~A, dax: ~A~%" a x p q dax)
    ;; (when (null p)
    ;;   (error "returning nil for p for a=~F, x=~F" a x))
    ;; (when (null q)
    ;;   (error "returning nil for q for a=~F, x=~F" a x))
...
;; This was an attempt to port the Boost PDF calculation to CL. It
;; turns out that calculating directly using the existing gamma is
;; both easier, and have results that match online calculators more
;; closely

;; Most of the error, as the comment in the Boost code suggests,
;; occurs in regularised-gamma-prefix function. Compare the calculated
;; version, rgp, with the regularised-gamma-prefix. The approximations
;; agree to the first 8 decimal points with the reference values
;; (online calculators and R), and the calculated versions (e.g. rgp)
;; agree to 16 decimals.
#|
...
(defun regularised-gamma-prefix (a x)
  ...
  (declare (double-float a x))
  (let (;(lanczos-g 6.024680040776729583740234375d0)
	(lanczos-g lanczos::g)
    ...
    (setf fac (+ a (- lanczos-g 0.5))
	  res (/ (sqrt (/ fac e))

		 ;; 5 digits of accuracy, with lanczos-g = lanczos::g (808618867/134217728) OR lanczos-g = 6.024 ...
		 ;;(lanczos::lanczos-sum a)

		 ;; 14/15 digits of accuracy, with lanczos-g = 6.024...
		 ;; 8 digits of accuracy using lanczos::g, the rational equivalent -- why?
		 (cephes:lanczos-sum-scaled a)
	...
	(progn
	  ;; (format t "x > 200~%")
	  (setf num (- x a (+ lanczos-g 0.5))
		res (* res
		       (exp (+ (* a (nu:log1pmx (/ num fac)))
			       (* x (- 0.5 lanczos-g) (/ fac))))))))
    res))

;;; This works for all tests
(cffi:defcfun ("igam_fac" regularised-gamma-prefix*) :double
  "Return x^a * exp(-x) / gamma(a)"
  (a :double)(x :double))

;;; This came from https://gitlab.common-lisp.net/oct/oct/-/blob/master/src/oct-gamma.lisp#L388
(defun regularised-gamma-prefix+ (a z)
  ...
  (/ (handler-case (* (expt z a)
	 ...
	 ;; form by default because it has more roundoff error than
	 ;; the former.
	 (exp (- (* a (log z)) z))))
     (gamma a)))


#| Accurate, but not across a large enough range to pass all the boost tests
(defun regularised-gamma-prefix+ (a z)
  (/ (* (expt z a)
	(exp (- z)))
     (gamma a))) |#
...
;;; This passes all Boost tests, if using cephes:lanczos-sum-scaled
(defun regularised-gamma-prefix- (a z)
...
(defun gamma-p-derivative (a x)
  ...
  (let ((f1 (regularised-gamma-prefix a x)))
    (cond ((<= a 0) (error "Argument a to the incomplete gamma function must be greater than zero (got a=~S)." a))
	  ...
	  ;; ((and (< x 1)
	  ;; 	(< f1 (* x max...
	  ;; See https://github.com/boost-cmake/boost/blob/527022e770e6fe217d849e72857e84e1610ab89f/libs/math/include/boost/math/special_functions/gamma.hpp#L1618
	  (t (/ f1 x)))))


;; This one gives the same values as the online calculators
(defun gamma-pdf (x shape scale)
...
;;; This agrees to 8 digits of accuracy with the online
;;; calculators.
(defun gamma-pdf* (x a b)			;a = shape, b = scale
...
Returns: x^(a-1)*exp(-x/b)/gamma(a)/b^a, x>0"
  ...
  (cond ((or (<= a 0)
	     (<= b 0)
	     (< x 0))
	 (return-from gamma-pdf* (bits-double-float #b0111111111111000000000000000000000000000000000000000000000000000))) ;double-float-nan)) See SBCL bug
	((= x 0)
	 (cond ((= a 1)
		(/ b))
	       ((> a 1)
	...
	(t (/ (regularised-gamma-prefix a (/ x b)) x))))


#| I started to port this before I realised that a direct calculation,
   at least in Lisp-Stat, was equally accurate.  Leaving here in case it's needed later.
;;; The Chebyshev approximation
(defun gamma-aux* (x)
  "Temme's function g(x) in 1/Gamma(x) = 1 + x*(x-1)*g(x), x >= -1"
  (declare (double-float x))
  (let+ ((gah (make-array 17
			  :initial-contents '(-1.01360925800986577694906309789d0
					      +0.784903531024782283533943840994d-1
					      +0.675886687432583155292019772240d-2
					      -0.127904348696234681212975260904d-2
					      +0.462939838642739584694545688440d-4
					      +0.433816817447403519173829550977d-5
					      -0.532687242261800541117044891072d-6
					      +0.172233457410539543634464445150d-7
					      +0.830054210711905120609172324971d-9
					      -0.105539942399565611532163232305d-9
					      +0.394158428521837947892479562496d-11
					      +0.362068538620576498220306882292d-13
					      -0.107440227418271864750448383428d-13
					      +0.500041439419368462237596022981d-15
					      -0.624516672751596145046065537918d-17
					      -0.518509067074711462577736062188d-18
					      +0.346796669906221429536559013309d-19)
			  :element-type 'double-float))
	 ((&flet chev-expnd (x)
	    (nu:evaluate-chebyshev gah (1- (* 2 x))))))
    (cond ((<= x -1) (- 0.5d0))
	  ((< x 0) (- (/ (1+ (* (square (1+ x))
				(chev-expnd (1+ x))))
			 (- 1 x))))
